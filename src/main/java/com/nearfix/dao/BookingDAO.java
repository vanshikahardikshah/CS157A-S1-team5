...
public class BookingDAO {
    ...
    public Integer create(int customerId, int serviceId, int availabilityId) {
        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(
                    "SELECT provider_id, available_date FROM availability WHERE availability_id = ? FOR UPDATE")) {
            ...
            int bookingId;
            try (PreparedStatement ps = conn.prepareStatement(
                    "INSERT INTO bookings (customer_id, service_id, booking_date, status, total_price) VALUES (?, ?, ?, 'pending', ?)",
            ...
            try (PreparedStatement ps = conn.prepareStatement("DELETE FROM availability WHERE availability_id = ?")) {
                ps.setInt(1, availabilityId);
                ps.executeUpdate();
            }
    ...
    public boolean cancelByCustomer(int bookingId, int customerId) {
        String sql = "UPDATE bookings SET status = 'cancelled' WHERE booking_id = ? AND customer_id = ? AND status IN ('pending', 'confirmed')";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, bookingId);
    ...
    private Booking mapRow(ResultSet rs) throws SQLException {
        ...
        try { b.setServiceName(rs.getString("service_name")); } catch (SQLException ignored) {}
        try { b.setCustomerName(rs.getString("customer_name")); } catch (SQLException ignored) {}
        try { b.setProviderBusinessName(rs.getString("business_name")); } catch (SQLException ignored) {}
        return b;
    }
}
